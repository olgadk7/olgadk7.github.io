...
category: project
author: Olga Kahn
summary: "Comparing feature-selection methods — intuition, correlation, PCA, RFE, and Lasso — to predict house prices in the Ames, Iowa dataset."
...
I reran an old project from my General Assembly bootcamp and, knowing more than I did the first time, ended up rebuilding it from scratch. The premise is a small contest: my judgment versus the machine's.

The Ames housing dataset is famously unruly — 80 features and a swamp of missing values — so the real work here isn't prediction, it's deciding *which features matter*. I lined up five ways to choose them: my own common sense (the 24 features we all assume drive house prices), Pearson correlation, PCA, Recursive Feature Elimination, and Lasso.

Then I let them compete. **The human lost.** My hand-picked features predicted worse than every statistical method, with PCA coming out on top. The modelling itself is deliberately bare-bones — I didn't want hyperparameter tuning to steal attention from the feature-selection contest — and I fold the whole process into a single pipeline at the end.
...
# dropping the relatively/subjectively unimportant
...
print("%s variables of %s were removed"%(sum(lasso.coef_ == 0), len(lasso.coef_)),'\n')
...
```
    ...
    182 variables of 194 were removed 
