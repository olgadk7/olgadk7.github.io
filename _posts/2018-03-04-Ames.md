...
3. Principal Component Analysis, the most popular dimensionality reduction algorithm that transforms original variables onto the new axes
...
Lastly, I've put all the processing steps in a pipeline, taking up one code block in the very end. The results in the pipeline are different, most likely due to its sweeping method of dealing with the missing values.
