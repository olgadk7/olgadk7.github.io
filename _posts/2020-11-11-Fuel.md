...
author: Olga Kahn
category: project
summary: "Modeling vehicle fuel economy from EPA data with Binomial and Gamma generalized linear models (GLMs) and different link functions."
permalink: projects/fuelefficiency
---

This one started as a take-home interview assignment: build a model explaining the fuel economy of every car the EPA has on record, in a single self-contained notebook, with a hiring manager as the audience. It took me well past the polite "3–4 hours" estimate — but it answered some genuinely fun questions. How has fuel economy changed over the decades? Which manufacturer builds the most efficient cars? And what actually moves city MPG? I work up to a Generalized Linear Model (Binomial and Gamma families, testing a few link functions) by way of the target's distribution, some exploratory analysis, and the trends over time.
...
![png](/assets/images/posts/final_files/final_120_0.png)
